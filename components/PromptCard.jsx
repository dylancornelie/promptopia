...
function PromptCard({ prompt, handleTagClick, handleEdit, handleDelete }) {
  ...
  return (
    <div className="prompt_card">
      ...
      {session?.user.id == prompt.creator._id && handleEdit != null && handleDelete != null && (
        <div className="mt-5 flex-center gap-4 border-t border-gray-100 pt-3">
          <p className="font-inter text-sm green_gradient cursor-pointer" onClick={() => handleEdit(prompt)}>Edit</p>
          <p className="font-inter text-sm orange_gradient cursor-pointer" onClick={() => handleDelete(prompt)}>Delete</p>
        </div>
      )}
    </div>
  )
